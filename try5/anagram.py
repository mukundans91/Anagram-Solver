...
def findana(inp,db):
	...
	cust=time.time()
	perm.initpermute()
	perm.showPattern("",inp)
	print perm.getpermute()
	custp=time.time()-cust
	return rows,elapsed,pelap,custp
...
while True:
	...
	if inp=='end!':
		break
	rows,elapsed,pelap,custp=findana(inp,db)
	if not rows:
		...
		print 'The time taken to find the permutations is :',pelap
		print 'The time taken to find the permutations using custom algorithm is :',custp
